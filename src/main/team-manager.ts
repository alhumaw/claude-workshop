...
export class TeamManager {
  ...
  private getMempalacePython(): string {
    // process.cwd() is the project root in Electron main process
    const cwd = process.cwd();
    const devVenv = path.join(cwd, 'working', 'mempalace', '.venv', 'bin', 'python');
    console.log(`[Workshop:mempalace] cwd=${cwd} devVenv=${devVenv} exists=${existsSync(devVenv)}`);
    if (existsSync(devVenv)) return devVenv;

    try {
      execFileSync('python3.13', ['-c', 'import mempalace'], { stdio: 'pipe', timeout: 5000 });
      console.log('[Workshop:mempalace] using python3.13');
      return 'python3.13';
    } catch {}

    try {
      execFileSync('python3', ['-c', 'import mempalace'], { stdio: 'pipe', timeout: 5000 });
      console.log('[Workshop:mempalace] using python3');
      return 'python3';
    } catch {}

    console.log('[Workshop:mempalace] WARNING: no mempalace python found, falling back to python3');
    return 'python3';
  }
}
